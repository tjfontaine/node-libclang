...
exports.generate = function (opts) {
  ...
  var defineType = function (type) {
    ...
    if (!mappedAbort && elements.length > 0) {
      serializer += 'var ' + type.spelling;
      serializer += ' = exports.' + type.spelling;
      ...
      serializer += 'var ' + type.spelling + 'Ptr ';
      serializer += ' = exports. ' + type.spelling + 'Ptr';
      serializer += ' = ref.refType(' + type.spelling + ');';

      structs[type.spelling] = new WrapType(type.spelling);
      return structs[type.spelling];
    } else {
      return undefined;
    }
  ...
  var defineOpaque = function (canonical) {
    ...
    ret.opaque = true;
    if (!structs[canonical]) {
      serializer += 'var ' + canonical;
      serializer += ' = exports.' + canonical;
      serializer += ' = StrictType(voidPtr);';
      structs[canonical] = ret;
    }
    return ret;
  };
  ...
  var defineFunction = function (name, type) {
    ...
    for (i = 0; i < type.argTypes; i++) {
      arg = mapType(type.getArg(i), name + '-arg' + i);
      if (!arg) {
        arg = type.getArg(i);
        ret.abort = {
          name: name,
          displayname: arg.declaration.displayname,
          arg: arg.spelling,
          position: i,
        };
        return ret;
